...
from `PDZ_Gripper_Slim(1).STEP`, exported from SolidWorks 2022 using the
...
- `pdz_gripper_tcp` is provisionally centered between the Slim fingertips at
  `[0, 0, 0.1505] m`. Change it if the intended grasp frame
  differs.
...
self-collisions with the guide rods. The pad meshes come from the 8 mm Slim
reference pads in the CAD assembly and are scaled only along their thickness
axis while keeping their mounting faces fixed.
...
revision because the fully-open finger X positions are unchanged. Verify these
against the assembled mechanism if the redesigned tensioner or belt routing
introduces a different mechanical stop.
